...
import argparse
import os
from sklearn.metrics import accuracy_score, roc_auc_score, matthews_corrcoef, precision_recall_curve, auc, recall_score, precision_score, f1_score, confusion_matrix, average_precision_score
...
ece, acc_in_bin, avg_conf_in_bin = eceloss(all_scaled_softmaxes_tensor, all_labels_tensor)



ece_scaled, acc_in_bin_scaled, avg_conf_in_bin_scaled = eceloss(all_scaled_softmaxes_tensor, all_labels_tensor)
print("Temperature-Scaled ECE:", ece_scaled.item())
print("Accuracy in each bin (scaled):", acc_in_bin_scaled)
print("Average confidence in each bin (scaled):", avg_conf_in_bin_scaled)
...
print(f'Test Accuracy: {accuracy:.4f}, ROCAUC: {roc_auc:.4f}, AUCPR:{auc_pr:.4f} MCC: {mcc:.4f}, F1: {F1:.4f}')
...
fisoftmaxes = all_scaled_softmaxes_tensor.cpu()
...
def evaluate_uncertainty_threshold(all_labels, all_preds, softmaxes, thresholds=[0.2, 0.4, 0.6, 0.8]):
    ...
    for cutoff in thresholds:
        ...
        roc_auc_filtered = roc_auc_score(T_filtered, Y_filtered)
        auc_pr_filtered = average_precision_score(T_filtered, Y_filtered)
        ...
        results[cutoff] = {
            ...
            "ROCAUC": roc_auc_filtered,
            "AUCPR": auc_pr_filtered,
            "MCC": mcc_filtered,
        ...
        print(f"  ROCAUC: {roc_auc_filtered:.4f}")
        print(f"  AUCPR: {auc_pr_filtered:.4f}")
...
scaled_softmaxes_np = all_scaled_softmaxes_tensor.cpu().numpy()
