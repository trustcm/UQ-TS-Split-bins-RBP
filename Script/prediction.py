...
model_path = os.path.normpath(os.path.join(args.output_model, 'best_model_fold_1.pth'))
...
ece, acc_in_bin, avg_conf_in_bin, ece_per_bin, bin_samples, bin_interval_list, bin_ece_list = eceloss(all_softmaxes_tensor, all_labels_tensor)
...
print(f'Test Accuracy: {accuracy:.4f},balanced_ACC:{balanced_accuracy:.4f}, ROCAUC: {roc_auc:.4f}, AUCPR:{auc_pr:.4f} MCC: {mcc:.4f}, F1: {F1:.4f}, recall: {recall:.4f}, pre: {pre:.4f}')
...
# top 2-,4-,6- bin's interval 
target_intervals_list = [
    [(0.4667, 0.5333), (0.9333, 1.0)],
    [(0.4667, 0.6), (0.8667, 1.0)],
    [(0.4667, 0.6667), (0.8, 1.0)]
]

for target_intervals in target_intervals_list:
    selected_indices = []
    confidences, _ = torch.max(all_softmaxes_tensor, 1)
    ...
    selected_softmaxes = all_softmaxes_tensor[torch.tensor(selected_indices)]
    ...
    print(f'recall for selected samples: {recall:.4f}, pre for selected samples: {pre:.4f}')
